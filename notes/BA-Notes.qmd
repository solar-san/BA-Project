...
3a. Estimate the _seasonal components_ by taking the average of all detrended value for each for that season^[E.g.: take the average of all de-trended January values.].
...
3a. Estimate the seasonal components by taking the average of all the detrended value for each for that season^[E.g.: take the average of all de-trended January values.].
...
- The estimate of the trend-cycle (and therefore the remainder) is unavailable for the first few and last few observations; moreover, it tends to oversmooth rapid raises and falls in the data.
...
```{r benchmark forecating google stock}
goog_stock %>%
  model(
    Mean = MEAN(Close),
    Naive = NAIVE(Close)
...
The last term becomes very small for large $T$; the forecast consist in the _weighted average_ of all observations.

### Smoothing Parameters Choice

The parameters to be estimated are $\alpha$ and $l_0$. Similarly to regression, we can choose optimal parameters by minimising the SSE^[Sum of Squared Errors.].
...
The principle is the same of SES: we can estimate a trend by giving more weight to more recent changes. There are two extreme situations:
...
## Models with Seasonality
 ...
 This approach is typically combined with model with trend; seasonality can enter the model either in an _additive_ (@eq-additive) or _multiplicative_ (@eq-multiplicative) way. It is called Triple Exponential Smoothing_ or _Holt-Winters method_.

### Holt-Winters Additive Method

:::{.callout-tip}
This model is appropriate when seasonal variations are independent to the level of the series.
:::
