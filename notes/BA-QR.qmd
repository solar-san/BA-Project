...
format:
  html:
    ...
    header-includes: |
      ...
      <meta 
      name="description"
      content="Notes on quantile regression for business analysts. It contains a self-contained introduction to the topic, with R code snippets and the necessary mathematical and statistical notation.">
...
- Symmetry.
- If the mean and scale are known, we can assume that we know the whole distribution. Therefore, knowing _how the mean changes_ explains _all the changes in the distribution_.

However, this is often not the case. 

> This dataset contains information from the 2001 panel of the _Survey of Income and Program Participation_ (SIPP). The dataset contains information on several American households and the household's annual income in 2001.
...
:::{callout.important}
Many variables are _not_ symmetric. Moreover, we are often not only interested in the _mean_, but in understanding _extremes_. Since the linear regression model cannot give the full description of the distribution and everything is described by its mean, we need an alternative Framework.
:::
...
Moreover, the difference between the 75th and the 50th quantile is larger than the difference between the 50th and the 25th percentile: this is an indication of an _asymmetry_ in the distribution. The same goes when comparing the more extreme tails (e.g. the 10th and 90th quantile): 

```{r computing quantile differences}
...
The quantiles can be _estimated_ by inverting the _ECDF_: this amounts to finding $Q(p)$ for a given $p$ and, even if the function is _discrete_, we can extrapolate between the jumps. However, we need to take into account the sampling distribution to make any inference about the characteristics of interest and their distribution shape in the population we are sampling from.
...
Quantiles allow us to describe the _scale_ and _shape_ of distributions.
...
:::{.callout-note}
Minimization is achieved with _linear programming_ methods. The solution _might be not unique_.
:::
...
> With this approach we can see that expenditure on food is _not_ the same in the different quantiles.
...
The difference between the two errors depends on $x_i$. If the two slopes are equal^[$(\beta_1^{(p)} - \beta_1^{(q)}) = 0$.], then the two errors differ only by a constant: hence, the quantile error distribution only differs for a constant. This means that the observations are identically distributed for different values of $X$.

:::{.callout-important}
QRM can be used to assess whether the assumption of _identically distributed_ observations is valid.
:::

Note that, however, to formally test the equality of slopes we need a way to make _inference_^[@sec-inference.]: if that was the case we would be in a situation in which we have the _same shape_ for the distribution conditional on the different values of the predictor^[For example, an indication that we are observing a conditionally identically distributed response variable.]. In other words, we need inference because we are interested in testing:
...
```{r}
anova(rqmultfit_engel)
```

We can see in this case we confidently reject the null hypothesis: the slopes are quite different and we would be inclined to consider that the conditional distribution of `foodexp` changes at different levels of `income`.

## Multiple Independent variables

We can extend the model and obtain a _multivariate_ quantile regression. In the (compact) matrix notation we have:
...
Again, we need a way to assess the significance of each added variable: in other words, a way to make _inference_^[See @sec-inference.].
...
We will fit a multivariate quantile regression with both variables:
...
We should always take a look at the residuals:
...
In general, the required mathematics is difficult and it is not always possible to derive asymptotic variances analytically. Hence, a simulation-based method such as the __bootstrap__ is required.

## The Bootstrap

> __Bootstrapping__^[First introduced by Bradley Efron in the late 70s.] is _any test or metric that uses random sampling with replacement_, mimicking the sampling process, and falls under the broader class of _resampling methods_. Bootstrapping assigns _measures of accuracy_^[Such as bias, variance, confidence intervals, prediction error, etc.] to sample estimates. This technique allows the estimation of the sampling distribution of almost any statistic using random sampling methods.

It is based on having sample data and _enough_ computing power. The key idea is that we assume the population is just many copies of the sample; then, we make lots of copies of the sample and then we sample repeatedly from the population. In practice, instead of making many copies of the sample and sampling from that, we use a sampling technique that is equivalent, based on _sampling with replacement_.

In particular, we will use __nonparametric resampling bootstrap__.
...
Inference on a single coefficient is often useful; however, we wish to test other hypotheses and also a to test _set_ of hypotheses.
...
We are interested in constructing a test to verify the following null hypothesis^[This test amounts to asking the question: "Are the added variables _significant_ in explaining the behavior of our dependent variable?"]:
...
:::{.callout-tip}
we should test for _more_ than two quantiles; formal testing gives _evidence_ of varying slopes.
:::

### Example: Bootstrap

> This first application of the bootstrap is characterized by a situation in which a well-established theory exists to quantify the variability of our estimation: _the estimation of the mean_.

We will keep using the Munich rent dataset restrict ourselves to the houses with _premium kitchens_, and focus on variables `area` and `rentsqm`:

```{r}
rents_kp <- rents[rents$kitchen ==1, ]
```
...
We can iterate the procedure to obtain a _distribution_ of means:
...
Moreover, we can compute _bootstrapped confidence intervals_:
...
To compute _bootstrapped standard errors_, based on resampling, we need to pass the argument `se = "boot"` to the `summary` function. `R` specifies the number of bootstrap replications.
...
The following code computes the bootstrapped confidence intervals and standard errors _stepwise_: this will help in understanding the logic behind this resampling method.

1. We need to initialize an empty matrix in which we will store the bootstrapped _intercept_ and _slope_ values.
...
2. Then, we can loop the sampling procedure and store the fitted coefficients in the initialized array.
...
boots <- as_tibble(boots)  
```

3. To conclude, we can compute the bootstrapped summary statistics.
...
p1 / p2 + plot_annotation(title = "Bootstrapped Coefficients")
```

## Example: Inference & Bootstrap 

We will compare different models; they will be _polynomial_ variations of the same regression.
...
The option `se = "boot"` computes the standard errors using the _bootstrap_. To compare the fitness of available models, we can use the ANOVA test:
...
Feeding the `anova` function with different $\tau$ for the same model will conduct a joint test for the equality of slopes:

```{r}
anova(rqfit2munich)
```

We can visualize the _predicted quantiles_ by looking at the effect of `area` on `rent` for different fixed values of `yearc`. 
We wish to show this in a _faceted_ plot, so we will make use of the `expand_grid` function from the `tidyr` package.
...
> Once we have the predicted values for different quantiles we can also plot some measure of _scale_ and _shape_ as a function of some explanatory variable to assess whether this is changing: we compute $QSC(0.25)$ and $QSK(0.1)$ for _fixed values_ of `yearc` and allowing `area` to vary:
