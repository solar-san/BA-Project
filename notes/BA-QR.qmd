...
format:
  html:
    ...
    header-includes: |
      ...
      <meta 
      name="description"
      content="Notes on quantile regression for business analysts. It contains a self-contained introduction the topic, with R code snippets and the necessary mathematical and statistical notation.">
...
Note that, however, to formally test the equality of slopes we need a way to make _inference_^[@sec-inference.]: if that was the case we would be in a situation in which we have the _same shape_ for the distribution conditional on the different values of the predictor^[For example, an indication of conditionally identically distributed response variable.]. In other words, we need inference because we are interested in testing:
...
We will keep using the Munich rent dataset and restrict ourselves on the houses with _premium kitchens_, and focus on variables `area` and `rentsqm`:
