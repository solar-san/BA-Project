...
It is based on the sample data and having _enough_ computing power. The key idea is that we assume the population is just many copies of the sample; then, we make lots of copies of the sample and then we sample repeatedly from the population. In practice, instead of making many copies of the sample and sampling from that, we use a sampling technique that is equivalent, based on _sampling with replacement_.
...
We will compare different models; they will _polynomial_ variations of the same regression.
...
> Once we have the predicted values for different quantiles we can also plot some measure of _scale_ and _shape_ as a function of some explanatory variable to assess whether this is changing: we compute $QSC(0.25)$ and $QSK(0.9)$ for _fixed values_ of `yearc` and allowing `area` to vary:
