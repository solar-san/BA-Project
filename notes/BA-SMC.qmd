...
While a closed-form solution is sometimes available for a large set of problems, provided that some quantities are known. This is often not the case. For example, _profits_ are a function of _revenues_ and _costs_: if the two latter quantities are known, then we can easily compute the _profits_; but revenues and costs are typically _not_ known precisely, although it is possible to make some _probabilistic assessment_ on the range of possible values.
...
> In many cases we are _uncertain_ about some of the inputs in our calculations, although we can assume that the input will follow a given distribution of values. These assumptions can be modeled mathematically using _stochastic variables_, instead of deterministic inputs and outputs.
